...
summary(glmfCO2n112 <- glm(fCO2~TOC,offset = fCO2.atm, data = df, family = Gamma(link = "log")))
...
summary(glmfCO2n112 <- glm(fCO2~TOC+offset(fCO2.atm), data = subset(nofin, survey == "N112_2004"), family = Gamma(link = "identity")))
coef.glmfCO2n112 <- summary(glmfCO2n112)$coefficients
r2n112 <- with(summary(glmfCO2n112), 1 - deviance/null.deviance) %>% round(2)
...
### fCO2 models with gaussian family
...
```
```{r fCO2-toc-cba}
summary(glmfCO2cba <- glm(fCO2~TOC, data = subset(nofin, survey == "CBA_2019"), family = gaussian(link = "identity")))
summary.glmfCO2cba <- summary(glmfCO2cba)
coef.glmfCO2cba <- summary(glmfCO2cba)$coefficients
r2cba <- with(summary.glmfCO2cba, 1 - deviance/null.deviance) %>% round(2)
...
ggplot(subset(nofin, survey == "CBA_2019"))+geom_point(aes(x = TOC, y = fCO2))+
  ...
  geom_line(aes(x = TOC, y = coef.glmfCO2cba[1]-coef.glmfCO2cba[3]+TOC*(coef.glmfCO2cba[2]-coef.glmfCO2cba[4]), col = "Std Error"), lty = 2)+
  scale_color_manual(values = c("dodgerblue3", "firebrick"))+
  annotate(x = 0.0003, y = 0.004, geom = "label", label = paste("Intercept = ", round(coef.glmfCO2cba[1]*1e6, 2),"\nSlope = ", round(coef.glmfCO2cba[2],3), "\n R2 = ", r2cba))+
  coord_trans(x="log10", y = "log10")+
  theme(legend.position = "none")+
  labs(y = "fCO2, atm", x = "TOC, mol/L", title = "CBA survey, 2019")+
  theme_minimal()
```
```{r fCO2-toc-nlss-gaussian}
summary(glmfCO2nlss <- glm(fCO2~TOC, data = nlss, family = gaussian(link = "identity")))
...
summary(glmfCO2nlss <- glm(fCO2~TOC+offset(-fCO2.atm), data = nlss, family = Gamma(link = "sqrt")))
coef.glmfCO2nlss <- summary(glmfCO2nlss)$coefficients
r2nlss <- with(summary(glmfCO2nlss), 1 - deviance/null.deviance) %>% round(2)

nlss$predfCO2_sqrt <- predict(glmfCO2nlss, se = T)[1] %>% unlist
nlss$predfCO2_se <- predict(glmfCO2nlss, se = T)[2] %>% unlist


ggplot(nlss, aes(x = TOC))+
  geom_point(aes(y = fCO2))+
    geom_line(aes(y = predfCO2_sqrt^2, col = "predict"))+
  geom_line(aes(y = (predfCO2_sqrt+predfCO2_se)^2, col = "Std Error"), lty = 2)+
  geom_line(aes(y = (predfCO2_sqrt-predfCO2_se)^2, col = "Std Error"), lty = 2)+
  scale_color_manual(values = c("dodgerblue3", "firebrick"))+
  geom_hline(yintercept = fCO2.atm.1995, col = "pink")+
  annotate(x = 0.00003, y = 0.04, geom = "label", 
           label = paste("Intercept =",round(((coef.glmfCO2nlss[1])^2)*1e6, 2)," uatm","\n Slope = ", round(coef.glmfCO2nlss[2],2), "\n R2 = ", r2nlss))+
  coord_trans(x="log10", y = "log10")+
  labs(y = "fCO2, atm", x = "TOC, mol/L", title = "NLS survey, 1995", col = "")+
  theme_minimal()+
    theme(legend.position = "bottom", 
...
ggplot(nlss, aes(x = TOC))+
  geom_point(aes(y = fCO2))+
    geom_line(aes(y = exp(predfCO2_log), col = "predict"))+
  geom_line(aes(y = exp(predfCO2_log+predfCO2_se), col = "Std Error"), lty = 2)+
  geom_line(aes(y = exp(predfCO2_log-predfCO2_se), col = "Std Error"), lty = 2)+
  scale_color_manual(values = c("dodgerblue3", "firebrick"))+
  geom_hline(yintercept = fCO2.atm.1995, col = "pink")+
  annotate(x = 0.00003, y = 0.04, geom = "label", 
           label = paste("Intercept =",round(exp(coef.glmfCO2nlss[1])*1e6, 2)," uatm","\n Slope = ", round(coef.glmfCO2nlss[2],), "\n R2 = ", r2nlss))+
  coord_trans(x="log10", y = "log10")+
  labs(y = "fCO2, atm", x = "TOC, mol/L", title = "NLS survey, 1995", col = "")+
  theme_minimal()+
    theme(legend.position = "bottom", 
...
```{r fCO2-toc-n112-offset}
df <- subset(nofin, survey == "N112_2004")

summary(glmfCO2n112 <- glm(fCO2~TOC+0+offset(fCO2.atm), data = df, family = Gamma(link = "identity")))
coef.glmfCO2n112 <- summary(glmfCO2n112)$coefficients
r2nn112 <- with(summary(glmfCO2n112), 1 - deviance/null.deviance) %>% round(2)
...
ggplot(df, aes(x = TOC))+
  ...
  scale_color_manual(values = c("dodgerblue3", "firebrick"))+
  geom_hline(yintercept = fCO2.atm.2019, col = "pink")+
  annotate(x = 0.0003, y = 0.003, geom = "label", 
...
### fCO2 with offset and gaussian distribution

```{r fCO2-toc-n112-offset}
df <- subset(nofin, survey == "N112_2004")

summary(glmfCO2n112 <- glm(fCO2~TOC+0+offset(fCO2.atm), data = df, family = gaussian(link = "identity")))
coef.glmfCO2n112 <- summary(glmfCO2n112)$coefficients
r2nn112 <- with(summary(glmfCO2n112), 1 - deviance/null.deviance) %>% round(2)
...
summary(glmfCO2cba <- glm(fCO2~TOC+0+offset(fCO2.atm), data = df, family = gaussian(link = "identity")))
coef.glmfCO2cba <- summary(glmfCO2cba)$coefficients
r2ncba <- with(summary(glmfCO2cba), 1 - deviance/null.deviance) %>% round(2)
...
summary(glmfCO2nlss <- glm(fCO2~TOC+0+offset(fCO2.atm), data = nlss, family = gaussian(link = "identity")))
coef.glmfCO2nlss <- summary(glmfCO2nlss)$coefficients
r2nlss <- with(summary(glmfCO2nlss), 1 - deviance/null.deviance) %>% round(2)
...
b6 <- bplot(nofin$DIC/nofin$TOC, "TIC/TOC")
...
```{r fit-evasion-rate-toc}
nofin$TOC_mg <- nofin$TOC*12.011*1e3
nofin$gC.m2.day <- nofin$mgC.m2.day*1e-3

summary(glmECO2 <- glm(gC.m2.day~TOC_mg, data = nofin, family = gaussian(link = "identity")))
...
ggplot(nofin, aes(x = TOC_mg))+
  ...
  coord_trans(x="log10")+
  labs(y = "ECO2, g/m2/day", x = "TOC, mg/L", title = "nofin", col = "")+
  theme_minimal()+
    theme(legend.position = "bottom", 
          axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
summary(glmECO2 <- glm(mgC.m2.day~TOC_mg, data = df, family = gaussian(link = "identity")))
coef.glmECO2 <- summary(glmECO2)$coefficients
r2ECO2 <- with(summary(glmECO2), 1 - deviance/null.deviance) %>% round(2)
...
ggplot(df, aes(x = TOC_mg))+
  geom_point(aes(y = mgC.m2.day))+
    geom_line(aes(y = (predECO2), col = "predict"))+
  geom_line(aes(y = (predECO2+predECO2_se), col = "Std Error"), lty = 2)+
  geom_line(aes(y = (predECO2-predECO2_se), col = "Std Error"), lty = 2)+
  scale_color_manual(values = c("dodgerblue3", "firebrick"))+
  #geom_hline(yintercept = fCO2.atm.1995, col = "pink")+
  annotate(x = 5, y = 15e3, geom = "label", 
           label = paste("Slope = ", round(coef.glmECO2[2],2), "\n R2 = ", r2ECO2))+
  coord_trans(x="log10")+
  labs(y = "ECO2", x = "TOC, mg/L", title = "CBA", col = "")+
  theme_minimal()+
    theme(legend.position = "bottom", 
          axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
ggplot(df, aes(x = TOC_mg))+
  ...
  scale_color_manual(values = c("dodgerblue3", "firebrick"))+
  #geom_hline(yintercept = fCO2.atm.1995, col = "pink")+
  annotate(x = 2, y = 3e3, geom = "label", 
           label = paste("Slope = ", round(coef.glmECO2[2],2), "\n R2 = ", r2ECO2))+
  coord_trans(x="log10")+
  labs(y = "ECO2", x = "TOC, mg/L", title = "N112", col = "")+
  theme_minimal()+
    theme(legend.position = "bottom", 
          axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
