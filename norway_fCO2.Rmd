...
summary(lmoaM <- glm(CA  ~ TA + invHplus:TOC + fCO2.atm, data = nofin, family = Gamma(link = "sqrt")))
...
ggplot(nofin)+geom_point(aes(x = log10(TOC), y = log10(fCO2*1e6), shape = survey))+
  ...
  labs(y = "log10(fCO2, ppm)", x = "log10(TOC, mol/L)", title = "CBA + N112 surveys")+
  theme_minimal()
...
ggplot(subset(nofin, survey == "CBA_2019"))+geom_point(aes(x = log10(TOC), y = log10(fCO2*1e6)))+
  ...
  theme(legend.position = "none")+
  labs(y = "log10(fCO2, ppm)", x = "log10(TOC, mol/L)", title = "CBA survey, 2019")+
  theme_minimal()
...
ggplot(subset(nofin, survey == "N112_2004"))+geom_point(aes(x = log10(TOC), y = log10(fCO2*1e6)))+
  ...
  theme(legend.position = "none")+
  labs("log10(fCO2, ppm)", x = "log10(TOC, mol/L)", title = "N112 survey, 2004")+
  theme_minimal()
...
ggplot(nofin)+
  ...
  facet_grid(rows = vars(survey))+
  labs(y = "log10(modelled fCO2, ppm)", x = "log10(measured fCO2, ppm)", col = "")+
  scale_color_manual(values = c("firebrick","orange"))+
  theme_minimal()
```
...
ggplot()+
  ...
  labs(size = "fCO2, ppm", col = "Survey", shape = "") +
...
ggplot()+
  ...
  geom_point(data = subset(nofin, dfCO2 > 0), aes(x=long, y = lat, size =mgC.m2.day, col = mgC.m2.day), shape = 16)+
  geom_point(data = subset(nofin, dfCO2 < 0), aes(x=long, y = lat, size =mgC.m2.day,col = mgC.m2.day), shape = 1)+
  labs(col = "Carbon evasion, in mgC/m2/day", size = "Carbon evasion, in mgC/m2/day") +
