...
ggplot(nlss, aes(x = pH))+geom_point(aes(y = log10(OA), col = log10(TA)))
...
summary(glmfCO2nlss <- glm(log10(fCO2*1e6)~TOC,data = nlss, family = Gamma(link = "identity")))
coef.glmfCO2nlss <- summary(glmfCO2nlss)$coefficients
r2nofin <- with(summary(glmfCO2nlss), 1 - deviance/null.deviance) %>% round(2)

ggplot(nlss, aes(x = TOC))+
  geom_point(aes(y = fCO2*1e6), shape = 1)+
...
```{r map-mgC.m2.day}
ggplot()+
  borders(region = c("Norway", "Sweden","Finland"), fill = "seashell3", col = "seashell3")+xlim(4,35)+ylim(55,72)+
