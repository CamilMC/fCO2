...
m <- 10.2*12.011*1e-6*1e-1# ueq/mg to eq/mol, site density BUT THERE IS A 1e3 PB
nlss$RCOO <- with(nlss, (Ka1/Hplus + 2*Ka2/Hplus + 3*Ka3/Hplus)* m/3 * TOC)

nlss$cTA <- with(nlss, TA + RCOO) #alkalinity corrected from organic acids

```

```{r boxplots}
nlss <- readRDS("nlss.rds")
nofin <- readRDS("nofin.rds")
bp.vars <- c("survey","pH", "TA", "TOC","lake.area")

df.boxplot <- rbind(dplyr::select(nlss, bp.vars), dplyr::select(nofin,bp.vars))

bplot <- function(input,xlab){
  ggplot(df.boxplot, aes(col = survey))+ 
...
b1 <- bplot(df.boxplot$pH, "pH")
b2 <- bplot(df.boxplot$TOC*12.011*1e3, "TOC in mg/L")
b3 <- bplot(log10(df.boxplot$TA*1e3), "log10(TA), meq/L")
b4 <- bplot(log10(df.boxplot$lake.area), "log10(Lake area), km2")
...
nlss$OA <- predict(lmoaM, newdata = nlss, type = "response") # TA corrected by OA

ggplot(nlss, aes(x = pH))+geom_point(aes(y = log10(sqrt(OA)), col = log10(TA)))
ggplot(nlss, aes(x = log10(TA)))+geom_point(aes(y = log10(OA), col = log10(TA)))+geom_abline(slope = 1, intercept = 0)

ggplot(nlss, aes(x = pH))+geom_point(aes(y = OA, col = TOC))

ggplot(nlss, aes(x = log10(TA)))+geom_point(aes(y = log10(sqrt(OA)), col = log10(TOC)))+
  geom_abline(slope = 1, intercept = 0)+
  scale_color_viridis_c()+theme_minimal()

ggplot(nlss, aes(x = TOC))+geom_point(aes(y = OA, col = log10(TA)))+
  geom_abline(slope = 1, intercept = 0)+
  scale_color_viridis_c()+theme_minimal()
...
```{r fCO2-corrected-TA}
nlss$CO3 <- with(nlss,(OA - OH + Hplus) / (Hplus/K2 +2)) # in mol/L
...
```{r plots-ca}
ggplot(nlss)+geom_point(aes(x = TA, y = CA, col = "corr TA"))+
  geom_point(aes(x = TA, y = ta_CA, col = "non corr TA"))+
  geom_abline(slope = 1, intercept = 0)+
  theme_minimal()


ggplot(nlss, aes(x=log10(TA)))+geom_point(aes(y = log10(CA), col = "corr TA"))+geom_vline(xintercept = log10(0.07e-3))+
  geom_point(aes(y = log10(ta_CA), col = "non corr TA"))+
  theme_minimal()
```

```{r plot fCO2-TOC}

ggplot(nlss, aes(x = log10(TOC), y = log10(fCO2)))+
  geom_hline(yintercept = log10(fCO2.1995*1e-6), col = "firebrick")+
  scale_color_manual(values = c("firebrick","lightblue","orange"))+
  geom_point()+
  theme_minimal()

ggplot(nlss) + geom_point(aes(x = log10(TOC), y = log10(fCO2), col = "corrected TA"), alpha = 0.5)+
  geom_point(aes(x = log10(TOC), y = log10(ta_fCO2), col = "non corrected TA"), alpha = 0.5)+
  geom_hline(yintercept = log10(fCO2.1995*1e-6), col = "firebrick")+
  scale_color_manual(values = c("firebrick","lightblue","orange"))+
  theme_minimal()

ggplot(nlss, aes(x = log10(TA))) + geom_point(aes(y = log10(fCO2), col = "corrected TA"), alpha = 0.5)+
  geom_point(aes(y = log10(ta_fCO2), col = "non corrected TA"), alpha = 0.5)+
  geom_hline(yintercept = log10(fCO2.1995*1e-6), col = "firebrick")+
  scale_color_manual(values = c("firebrick","lightblue","orange"))+
  theme_minimal()

ggplot(nlss, aes(x = pH)) + geom_point(aes(y = log10(fCO2), col = "corrected TA"), alpha = 0.5)+
  geom_point(aes(y = log10(ta_fCO2), col = "non corrected TA"), alpha = 0.5)+
  geom_hline(yintercept = log10(fCO2.1995*1e-6), col = "firebrick")+
...
ggplot(nlss, aes(x = pH)) + geom_point(aes(y = log10(fCO2), col = log10(TOC)), alpha = 0.5)+
  geom_hline(yintercept = log10(fCO2.1995*1e-6), col = "firebrick")+
...
ggplot(nlss, aes(x = pH))+geom_point(aes(y = H2CO3, col = "H2CO3"))+
  ...
  labs(title = "Carbonates from corrected alkalinity")+
  scale_color_manual(values = c("firebrick","lightblue","orange"))+
  theme_minimal()
...
ggplot(nlss, aes(x = pH))+geom_point(aes(y = ta_H2CO3, col = "H2CO3"))+
  geom_point(aes(y = ta_HCO3, col = "HCO3"))+
  geom_point(aes(y = ta_CO3, col = "CO3"))+
  labs(title = "Carbonates from measured alkalinity")+
  scale_color_manual(values = c("firebrick","lightblue","orange"))+
  theme_minimal()
...
```{r model-fCO2-toc}
summary(glmfCO2nlss <- glm(log10(fCO2*1e6)~log10(TOC), data = nlss, family = Gamma(link = "identity")))
coef.glmfCO2nlss <- summary(glmfCO2nlss)$coefficients
r2nofin <- with(summary(glmfCO2nlss), 1 - deviance/null.deviance) %>% round(2)

ggplot(nlss)+geom_point(aes(x = log10(TOC), y = log10(fCO2*1e6), shape = survey))+
  geom_line(aes(x = log10(TOC), y = coef.glmfCO2nlss[1]+log10(TOC)*coef.glmfCO2nlss[2], col = "predict"))+
  geom_line(aes(x = log10(TOC), y = coef.glmfCO2nlss[1]+coef.glmfCO2nlss[3]+log10(TOC)*(coef.glmfCO2nlss[2]+coef.glmfCO2nlss[4]), col = "Std Error"), lty = 2)+
  geom_line(aes(x = log10(TOC), y = coef.glmfCO2nlss[1]-coef.glmfCO2nlss[3]+log10(TOC)*(coef.glmfCO2nlss[2]-coef.glmfCO2nlss[4]), col = "Std Error"), lty = 2)+
  scale_color_manual(values = c("lightblue", "firebrick"))+
  scale_shape_manual(values = c(1,16))+
  annotate(x = -2.75, y = 2.3, geom = "label", label = paste("Slope = ", round(coef.glmfCO2nlss[2],2), "\n R2 = ", r2nofin))+
  theme(legend.position = "none")+
  labs(y = "log10(fCO2) in ppm", title = "Norhtern Lake surveys")+
  theme_minimal()

summary(glmfCO2nlss <- glm(log10(fCO2*1e6)~log10(TOC), data = nlss, family = Gamma(link = "log")))
r2nofin <- with(summary(glmfCO2nlss), 1 - deviance/null.deviance) %>% round(2)
nlss$predfCO2sqrt <- predict(glmfCO2nlss, type = "response") 

ggplot(nlss, aes(x = log10(TOC)))+geom_point(aes(y = log10(fCO2*1e6), col = "fCO2 from corrected TA"))+
  geom_point(aes(y = predfCO2sqrt, col = "fCO2 modelled"))+
  scale_color_manual(values = c("lightblue", "firebrick"))+
  scale_shape_manual(values = c(1,16))+
  annotate(x = -2.75, y = 2.3, geom = "label", label = paste("R2 = ", r2nofin))+
  theme(legend.position = "none")+
  labs(y = "log10(fCO2) in ppm", title = "Norhtern Lake surveys")+
  theme_minimal()
...
ggplot(nlss, aes(x = TOC))+
  geom_point(aes(y = fCO2), shape = 1)+
  geom_line(aes(y = coef.glmfCO2nlss[1]+TOC*coef.glmfCO2nlss[2], col = "predict"))+
  geom_line(aes(y = coef.glmfCO2nlss[1]+coef.glmfCO2nlss[3]+TOC*(coef.glmfCO2nlss[2]+coef.glmfCO2nlss[4]), col = "Std Error"), lty = 2)+
  geom_line(aes(y = coef.glmfCO2nlss[1]-coef.glmfCO2nlss[3]+TOC*(coef.glmfCO2nlss[2]-coef.glmfCO2nlss[4]), col = "Std Error"), lty = 2)+
  scale_color_manual(values = c("lightblue", "firebrick"))+
  annotate(x = 0.003, y = 0.0015, geom = "label", label = paste("Slope = ", round(coef.glmfCO2nlss[2],2), "\n R2 = ", r2nofin))+
  theme(legend.position = "none")+
  labs(y = "fCO2 in atm", title = "Norhtern Lake surveys")+
  theme_minimal()

nlss$predfCO2 <- predict(glmfCO2nlss, type = "response") 

ggplot(nlss, aes(x = log10(lake.area)))+geom_point(aes(y = fCO2, col = "fCO2 from corrected TA"))+
  geom_point(aes(y = predfCO2, col = "fCO2 modelled"))+
  scale_color_manual(values = c("lightblue", "firebrick"))+
  scale_shape_manual(values = c(1,16))+
  #annotate(x = -2.75, y = 2.3, geom = "label", label = paste("R2 = ", r2nofin))+
  theme(legend.position = "none")+
...
summary(glmfCO2nlss <- glm(fCO2~TOC+log10(lake.area),data = nlss, family = Gamma(link = "identity")))
coef.glmfCO2nlss <- summary(glmfCO2nlss)$coefficients
r2nofin <- with(summary(glmfCO2nlss), 1 - deviance/null.deviance) %>% round(2)
...
ggplot(nlss, aes(x = TOC))+geom_point(aes(y = fCO2, col = "fCO2 from corrected TA"))+
  geom_point(aes(y = predfCO2, col = "fCO2 modelled"))+
  scale_color_manual(values = c("lightblue", "firebrick"))+
  scale_shape_manual(values = c(1,16))+
  #annotate(x = -2.75, y = 2.3, geom = "label", label = paste("R2 = ", r2nofin))+
...
```{r model-tafCO2-toc}
summary(glmfCO2nlss <- glm(log10(ta_fCO2*1e6)~log10(TOC), data = nlss, family = Gamma(link = "identity")))
coef.glmfCO2nlss <- summary(glmfCO2nlss)$coefficients
r2nofin <- with(summary(glmfCO2nlss), 1 - deviance/null.deviance) %>% round(2)

ggplot(nlss)+geom_point(aes(x = log10(TOC), y = log10(ta_fCO2*1e6), shape = survey))+
  geom_line(aes(x = log10(TOC), y = coef.glmfCO2nlss[1]+log10(TOC)*coef.glmfCO2nlss[2], col = "predict"))+
  geom_line(aes(x = log10(TOC), y = coef.glmfCO2nlss[1]+coef.glmfCO2nlss[3]+log10(TOC)*(coef.glmfCO2nlss[2]+coef.glmfCO2nlss[4]), col = "Std Error"), lty = 2)+
  geom_line(aes(x = log10(TOC), y = coef.glmfCO2nlss[1]-coef.glmfCO2nlss[3]+log10(TOC)*(coef.glmfCO2nlss[2]-coef.glmfCO2nlss[4]), col = "Std Error"), lty = 2)+
  scale_color_manual(values = c("lightblue", "firebrick"))+
...
summary(glmfCO2nlss <- glm(ta_fCO2~TOC, data = nlss, family = Gamma(link = "log")))
coef.glmfCO2nlss <- summary(glmfCO2nlss)$coefficients
r2nofin <- with(summary(glmfCO2nlss), 1 - deviance/null.deviance) %>% round(2)

ggplot(nlss, aes(x = TOC))+geom_point(aes(y = ta_fCO2*1e6), shape = 1)+
  geom_line(aes(y = exp(coef.glmfCO2nlss[1]+TOC*coef.glmfCO2nlss[2]), col = "predict"))+
  geom_line(aes(y = exp(coef.glmfCO2nlss[1]+coef.glmfCO2nlss[3]+TOC*(coef.glmfCO2nlss[2]+coef.glmfCO2nlss[4])), col = "Std Error"), lty = 2)+
  geom_line(aes(y = exp(coef.glmfCO2nlss[1]-coef.glmfCO2nlss[3]+TOC*(coef.glmfCO2nlss[2]-coef.glmfCO2nlss[4])), col = "Std Error"), lty = 2)+
  scale_color_manual(values = c("lightblue", "firebrick"))+
  annotate(x = 2.5*1e-3, y = 1e5, geom = "label", label = paste("Slope = ", round(coef.glmfCO2nlss[2],2), "\n R2 = ", r2nofin))+
  theme(legend.position = "none")+
  labs(y = "log10(fCO2) in atm", title = "Norhtern Lake surveys")+
  theme_minimal()
...
nlss$Sc <- 1841*exp(-0.0549*nlss$temp_c) # @kokic2014
...
nlss$mgC.m2.day <- with(nlss, kCO2*(fCO2-fCO2.atm)*K0*1e3 *12.011 * 1e3) # mgC/m2/day
```

```{r maps}
ggplot()+
  geom_point(data = nlss, aes(x=long, y = lat, col = TOC*12.011*1e3))+
  borders(region = c("Norway","Sweden","Finland"))+xlim(4,37)+ylim(55,72)+
  scale_color_continuous_sequential(rev = T,  palette = "Reds")+labs(col = "TOC in mg/L")+ theme_void()

ggplot()+
  geom_point(data = filter(nlss, dfCO2 < 0), aes(x=long, y = lat, col = fCO2/fCO2.atm * 100))+
  borders(region = c("Norway","Sweden","Finland"))+xlim(4,37)+ylim(55,72)+
  scale_color_viridis_c() +
  theme_void()
...
ggplot()+
  ...
  geom_point(data = subset(nlss, dfCO2 > 0), aes(x=long, y = lat, size =log10(mgC.m2.day), col = log10(mgC.m2.day)), shape = 16)+
  geom_point(data = subset(nlss, dfCO2 < 0), aes(x=long, y = lat, size =log10(mgC.m2.day),col = log10(mgC.m2.day)), shape = 1)+
  labs(col = "Log10(Carbon evasion, mgC/m2/day)", size = "Log10(Carbon evasion, mgC/m2/day)", title = "Northern Lakes Survey") +
  scale_color_continuous_sequential(rev = T,  palette = "Reds")+
  scale_size(range = c(0.1,1))+
  guides(color = guide_legend(), size = guide_legend())+
  annotate(x = 32, y =5, geom = "text", label = paste("Median = ", round(median(nlss$mgC.m2.day, na.rm = T),2)))+
