...
```{r model-fCO2-toc-link-identity}
...
ggplot(nlss, aes(x = TOC))+
  ...
  geom_line(aes(y = coef.glmfCO2nlss[1]-coef.glmfCO2nlss[3]+TOC*(coef.glmfCO2nlss[2]-coef.glmfCO2nlss[4]), col = "Std Error"), lty = 2)+
  scale_color_manual(values = c("lightblue", "firebrick"))+
  annotate(x = 0.003, y = 0.15, geom = "label", label = paste("Slope = ", round(coef.glmfCO2nlss[2],2), "\n R2 = ", r2nofin))+
  theme(legend.position = "none")+
...
```{r glm-fCO2-toc-link-log}

summary(glmfCO2nlss <- glm(fCO2~TOC,data = nlss, family = Gamma(link = "log")))
coef.glmfCO2nlss <- summary(glmfCO2nlss)$coefficients
r2nofin <- with(summary(glmfCO2nlss), 1 - deviance/null.deviance) %>% round(2)

nlss$predfCO2 <- predict(glmfCO2nlss, type = "response") 
nlss$predfCO2_log <- predict(glmfCO2nlss, se = T)[1] %>% unlist
nlss$predfCO2_se <- predict(glmfCO2nlss, se = T)[2] %>% unlist

ggplot(nlss, aes(x = TOC))+
  geom_point(aes(y = fCO2))+
  geom_point(aes(y = predfCO2, col = "fCO2 modelled"))+
  geom_line(aes(y = exp(predfCO2_log + predfCO2_se), col = "Std"))+
  geom_line(aes(y = exp(predfCO2_log - predfCO2_se), col = "Std"))+
  scale_color_manual(values = c("lightblue", "firebrick"))+
  scale_shape_manual(values = c(1,16))+
