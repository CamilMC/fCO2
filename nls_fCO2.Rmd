...
df.boxplot <- rbind(select(nlss, bp.vars), select(nofin,bp.vars))
...
ggplot(nlss, aes(x = pH))+geom_point(aes(y = OA, col = log10(TA)))
...
ggplot(nlss, aes(x=log10(TA)))+geom_point(aes(y = log10(CA), col = "corr TA"))+
...
ggplot(nlss, aes(x = TOC))+geom_point(aes(y = fCO2*1e6), shape = 1)+
...
nlss$Sc <- getSchmidt(nlss$temp_c, "CO2")
...
nlss$k600 <- with(nlss, 2.51+1.48*nws_m.s+0.39*nws_m.s*log10(lake.area)) # lake.area already in km2
                    
nlss$kCO2 <- with(nlss, k600*(600/Sc)^n)

nlss$FCO2 <- with(nlss, lake.area*1e3*kCO2*((fCO2 - fCO2.atm)*K0*1e-3)) #-> mol/day
```
