...
bdg.niva <- readRDS("bdg.niva.rds") %>% dplyr::select(c("CBA__ID", "lake_area_km2","basin_area_km2"))
...
gca <- gam(DIC~s(I), method = "REML") %>% summary()
gca2 <- gam(DIC~s(I)+s(TOC), method = "REML") %>% summary()
```
