...
corrplot::corrplot.mixed(cor(dplyr::select(wtc, c("fCO2","ph","alk","ca","mg","na","k","cl","so4","f","no3","nh4","tp","tn","toc"))), tl.cex = 1.5, number.cex = 1.5)
```
...
ggplot(logwtc,aes(x=log10toc))+geom_point(aes(y=log10fCO2, col = "Obs"))+geom_point(aes(y = fpg, col = "Pred"))+scale_color_manual(values = c("lightblue","black"))+theme_minimal()
...
ggplot(wtc,aes(x=log10(toc)))+geom_point(aes(y=log10(fCO2), col = "Obs"))+geom_point(aes(y = log10(fpgc), col = "Pred"))+scale_color_manual(values = c("lightblue","black"))+theme_minimal()
...
GAM: linear pred with intercept. 
LM = GLM id link = Tweedie id link
...
t2 <- gam(list(fCO2~toc+NDVI+so4,~1, ~1), data = wtc, family = twlss(link = list("log","identity","identity")))
summary(t2)

t2b <- glm(fCO2~toc*tn+Alt, data = wtc, family = Gamma(link = "log"))
summary(t2b)

wtc$fp <- predict(t2b, type = "response")
ggplot(wtc,aes(x=log10(toc)))+geom_point(aes(y=log10(fCO2), col = "Obs"))+geom_point(aes(y = log10(fp), col = "Pred"))+
  scale_color_manual(values = c("lightblue","black"))+theme_minimal()

t2c <- glm(fCO2~., data = wtc, family = Gamma(link = "log"))
summary(t2c)
stepglm <- stepAIC(t2c, direction = "both", trace = FALSE)
summary(stepglm)
