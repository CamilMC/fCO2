...
```{r load-73-lakes, eval = F}
bdg.niva <- readRDS("bdg.niva.rds")

cba <- readxl::read_xlsx("CBA_100Lakes_Master.xlsx")

ggplot(cba)+geom_point(aes(x=pH,y=pH_Kje, col = log(TOC)))+geom_abline(slope = 1, intercept = 0)+scale_color_viridis_c()+theme_minimal()
ggplot(cba)+geom_point(aes(x=EC,y=EC_Kje, col = log(TOC)))+geom_abline(slope = 1, intercept = 0)+scale_color_viridis_c()+theme_minimal()
ggplot(cba)+geom_point(aes(x=pH,y=EC, col = log(TOC)))+scale_color_viridis_c()+theme_minimal()
ggplot(cba)+geom_point(aes(x=pH_Kje,y=EC_Kje, col = log(TOC)))+scale_color_viridis_c()+theme_minimal()

cba.total <- merge(cba,bdg.niva, by.x = "Lake_ID", by.y = "CBA_Lake_ID")

c1 <- ggplot(cba.total, aes(x=pH_Kje))+geom_point(aes(y = ph_2019, col = "NIVA"))+
  geom_point(aes(y = pH, col = "IBV"))+geom_smooth(aes(y=ph_2019, col = "NIVA"), formula = y~x, se = F, method = "lm")+
  geom_smooth(aes(y=pH, col = "IBV"), formula = y~x, se = F, method = "lm")+
  geom_abline(slope = 1, intercept = 0)+
  theme_minimal()

c2 <- ggplot(cba.total, aes(x=EC_Kje))+geom_point(aes(y = kond_2019*10, col = "NIVA"))+
  geom_point(aes(y = EC, col = "IBV"))+geom_smooth(aes(y=kond_2019*10, col = "NIVA"), formula = y~x, se = F, method = "lm")+
  geom_smooth(aes(y=EC, col = "IBV"), formula = y~x, se = F, method = "lm")+
  geom_abline(slope = 1, intercept = 0)+
  theme_minimal()

ggplot(cba.total)+geom_point(aes(x=Alkalinity, y = alke))+theme_minimal()
ggplot(cba.total)+geom_point(aes(x=Ca, y = ca_2019))+theme_minimal()
ggplot(cba.total)+geom_point(aes(x=Mg, y = mg_2019))+theme_minimal()
ggplot(cba.total)+geom_point(aes(x=K, y = k_2019))+theme_minimal()
ggplot(cba.total)+geom_point(aes(x=Na, y = na_2019))+theme_minimal()
ggplot(cba.total)+geom_point(aes(x=as.numeric(F), y = f_2019))+theme_minimal()
ggplot(cba.total)+geom_point(aes(x=Cl, y = cl_2019))+theme_minimal()
ggplot(cba.total)+geom_point(aes(x=SO4, y = so4_2019))+theme_minimal()
ggplot(cba.total)+geom_point(aes(x=as.numeric(NO3), y = no3_2019))+theme_minimal()
ggplot(cba.total)+geom_point(aes(x=Cl, y = cl_2019))+theme_minimal()
ggplot(cba.total)+geom_point(aes(x=SO4, y = so4_2019))+theme_minimal()


cba.sel <- cba.total %>% dplyr::select(c("Lake_ID","CO2","basin_area_km2","lake_area_km2","latitude","longitude","agriculture","peat","forest","era_runoff_1981_2010","era_temp_1981_2010","era_prec_1981_2010","nilu_ndep_2012_2016","pH_Kje","Alkalinity","EC_Kje","Ca","Mg","K","Na","Cl","F","SO4","NO3","TOTP","TN","TOC","T","geometry"))
...
cba.sel$mg <- cba.sel$Mg / 23.305 * 2 * 1000
cba.sel$k <- cba.sel$K / 39.098 * 1000
cba.sel$na <- cba.sel$Na / 29.990 * 1000
cba.sel$cl <- cba.sel$Cl / 35.45 * 1000
cba.sel$f <- cba.sel$F / 18.998 * 1000
...
cba.sel$alk <- cba.sel$Alkalinity * 10^3 #en ueq/L
cba.sel$cond <- cba.sel$EC_Kje

cba.sel$temp <- cba.sel$T + 273.15
cba.sel$ph <- cba.sel$pH_Kje
...
fCO2 in 2019: 410 ppm <https://www.eea.europa.eu/ims/atmospheric-greenhouse-gas-concentrations>

```{r model-fCO2-CBA, cache = T, cache.extra=file.mtime("cba.sel.rds")}
cba.sel <- readRDS("cba.sel.rds")
fCO2.2019.atm <- 410.5  * 10^6
temp <- cba.sel$T
TA <- cba.sel$alk * 10^(-6) # convert from ueq/L to mol/L
pH <- cba.sel$pH
CO2 <- cba.sel$CO2 * 10^(-6)
ae3 <- aquaenv(S=0, t=temp,pH = pH, TA=TA, SumCO2 = NULL, fCO2atm = fCO2.2019.atm)
cba.sel$fCO2 <- ae3$fCO2 *10^6
cba.sel$SumCO2 <- ae3$SumCO2 * 10^6 # conversion to uM
cba.sel$CO2_ae <- ae3$CO2 * 10^6 # conversion to uM

ggplot(cba.sel)+geom_point(aes(x=CO2,y=SumCO2, col= pH))+
  scale_color_viridis_c()+theme_minimal()

ggplot(cba.sel)+geom_point(aes(x=CO2,y=CO2_ae, col = pH))+
  geom_abline(slope = 1, intercept = 0)+
  scale_color_viridis_c()+theme_minimal()

ggplot(filter(cba.sel, ph > 5))+geom_point(aes(x=CO2,y = fCO2, col = log10(alk)))+
  scale_color_viridis_c()+theme_minimal()

ggplot(cba.sel)+geom_point(aes(x=ph, y = fCO2))+theme_minimal()


saveRDS(cba.sel, "cba.fco2.rds")

```

## Correction with Organic Alkalinity
...
cba.sel$orgalk <- cba.sel$TOC / 12.011 * 10^3 * 0.08 # TOC in mg/L converted to umol/L

ggplot(cba.sel, aes(x = alk, y = orgalk, col = toc))+geom_point()+scale_color_viridis_c()+
 geom_abline(slope =1, intercept = 0)+theme_minimal()

ggplot(cba.sel, aes(x = alk, y = toc, col = orgalk))+geom_point()+scale_color_viridis_c()+
 geom_abline(slope =1, intercept = 0)+theme_minimal()

fCO2.2019.atm <- 410.5  * 10^6
temp <- cba.sel$T
IS <- cba.sel$cond * 1.3 * 1e-5
pH <- cba.sel$pH - 0.03+0.05*log10(IS)

TA <- (cba.sel$alk - cba.sel$orgalk) * 10^(-6) # ueq/L to mol/L

ae4 <- aquaenv(S=0, t=temp,pH = pH, TA=TA, SumCO2 = NULL, fCO2atm = fCO2.2019.atm)
cba.sel$fCO2_oa <- ae4$fCO2 *10^6
cba.sel$SumCO2_oa <- ae4$SumCO2 * 10^6 # conversion to uM
cba.sel$CO2_oa <- ae4$CO2 * 10^6 # conversion to uM

ggplot(cba.sel)+geom_point(aes(x=CO2,y=SumCO2_oa, col= toc))+
  scale_color_viridis_c()+theme_minimal()

ggplot(cba.sel)+geom_point(aes(x=CO2,y=CO2_oa, col = toc))+
  geom_abline(slope = 1, intercept = 0)+
  scale_color_viridis_c()+theme_minimal()

ggplot(filter(cba.sel, ph > 6))+geom_point(aes(x=CO2,y=CO2_oa, col = toc))+
  geom_abline(slope = 1, intercept = 0)+
  scale_color_viridis_c()+theme_minimal()

ggplot(cba.sel)+geom_point(aes(x=CO2,y = fCO2_oa, col = log10(alk)))+
  scale_color_viridis_c()+theme_minimal()

ggplot(cba.sel)+geom_point(aes(x=ph, y = fCO2_oa, col = toc))+scale_color_viridis_c()+theme_minimal()


```

## Correction excess cations

### Compute xs

```{r tom-xs-cation}

cations <- with(cba.sel, na + k + ca + mg + (10^(6 - ph))) # don't have nh4 + converstion ph to [H+] in ueq/L
anions <- with(cba.sel, so4 + cl + f + no3 + alk)

qplot(y = cations, x = anions, col = cba.sel$tn)+scale_color_viridis_c()+
  geom_abline(slope = 1, intercept = 0, col = "red")+theme_minimal() # doesn't look like nh4 is the cause for too little cations

cba.sel$cat.xs <- cations - anions
cba.sel$cat.xs[cba.sel$cat.xs < -10000] <- NA 

library(MASS)

summary(m <- rlm(cat.xs ~ toc, data=cba.sel, method="MM")) # Robust linear model
#summary(m <- lm(cat.xs ~ toc, data=cba.sel)) # With LM, 0.016 ueq/mmol

ggplot(cba.sel)+geom_point(aes(x=toc, y = cat.xs))+geom_abline(slope = coef(m)[2], intercept = coef(m)[1], col = "red")+
  annotate(geom = "label", label = round(coef(m)[2] * 12.011 / 1000, 3), x = 60, y = -250)+
  theme_minimal()

coef(m)[2] * 12.011 / 1000 # 0.10 µeq org-TA / µmol TOC for NLS, here 0.03


```

### excess cation, removing high TOC

```{r tom-xs-cation-rm-high-toc}
cba.2 <- cba.sel %>% filter(toc < 40)

cations <- with(cba.2, na + k + ca + mg) # don't have nh4 + converstion ph to [H+] in ueq/L
anions <- with(cba.2, so4 + cl + f + no3)

qplot(y = cations, x = anions, col = cba.2$tn)+scale_color_viridis_c()+
  geom_abline(slope = 1, intercept = 0, col = "red")+theme_minimal() # doesn't look like nh4 is the cause for too little cations

cba.2$cat.xs <- cations - anions
cba.2$cat.xs[cba.2$cat.xs < -10000] <- NA 

library(MASS)

summary(m2 <- rlm(cat.xs ~ toc, data=cba.2, method="MM")) # Robust linear model
#summary(m2 <- lm(cat.xs ~ toc, data=cba.2)) # using LM with find a slope of 0.086 ueq/umol

ggplot(cba.2)+geom_point(aes(x=toc, y = cat.xs))+geom_abline(slope = coef(m2)[2], intercept = coef(m2)[1], col = "red")+
  annotate(geom = "label", label = round(coef(m2)[2] * 12.011 / 1000, 3), x = 20, y = -500)+
  theme_minimal()


coef(m2)[2] * 12.011 / 1000 # 0.10 µeq org-TA / µmol TOC for NLS, here 0.03


```

### fcO2 with xs cations

```{r orgalk-cations}

cba.sel$orgalk.xs <- cba.sel$TOC / 12.011 * 10^3 * 0.038 # TOC in mg/L converted to mmol/L

ggplot(cba.sel, aes(x = alk, y = orgalk.xs, col = toc))+geom_point()+scale_color_viridis_c()+
 geom_abline(slope =1, intercept = 0)+theme_minimal()

fCO2.2019.atm <- 410.5  * 10^6
temp <- cba.sel$T
TA <- (cba.sel$alk - cba.sel$orgalk.xs)*10^(-6) # ueq/L to mol/L

IS <- cba.sel$cond * 1.3e-5
pH <- cba.sel$pH - 0.03+0.05*log10(IS)
CO2 <- cba.sel$CO2 * 10^(-6)
ae5 <- aquaenv(S=0, t=temp,pH = pH, TA=TA, SumCO2 = NULL, fCO2atm = fCO2.2019.atm)
cba.sel$fCO2_oaxs <- ae5$fCO2 *10^6
cba.sel$SumCO2_oaxs <- ae5$SumCO2 * 10^6 # conversion to uM
cba.sel$CO2_oaxs <- ae5$CO2 * 10^6 # conversion to uM

ggplot(cba.sel)+geom_point(aes(x=CO2,y=SumCO2_oaxs, col= pH))+
  scale_color_viridis_c()+theme_minimal()

ggplot(cba.sel)+geom_point(aes(x=CO2,y=CO2_oaxs, col = pH))+
  geom_abline(slope = 1, intercept = 0)+
  scale_color_viridis_c()+theme_minimal()

ggplot(cba.sel)+geom_point(aes(x=CO2,y = fCO2_oaxs, col = log10(toc)))+
  scale_color_viridis_c()+theme_minimal()

ggplot(cba.sel)+geom_point(aes(x=ph, y = fCO2_oaxs))+theme_minimal()
...
```{r compare-fCO2}
ggplot(cba.sel, aes(x = fCO2))+geom_point(aes(y=fCO2_oa, col = "Liu"))+geom_point(aes(y = fCO2_oaxs, col = "xs cations"))+
  geom_abline(slope = 1, intercept = 0)+
  scale_color_viridis_d(end = 0.8)+theme_minimal()


ggplot(cba.sel, aes(x = CO2))+geom_point(aes(y=CO2_oa, col = "Liu"))+
  geom_point(aes(y = CO2_oaxs, col = "xs cations"))+
  geom_point(aes(y=CO2_ae, col = "no correction"))+
  geom_abline(slope = 1, intercept = 0)+
  scale_color_viridis_d(end = 0.8, option = "A")+theme_minimal()

```

### My aquaenv

Practical salinity unit: <http://www.salinityremotesensing.ifremer.fr/sea-surface-salinity/definition-and-units>
...
# Variables
pH <- cba.sel$pH
CO2 <- cba.sel$CO2 * 10e-6 # umol/L to mol/kg_solution
R <- 0.0820578 # L.atm/mol.K
pCO2 <- CO2 * R * cba.sel$temp # with temp in K
temp <- cba.sel$T # in celsius
TA <- cba.sel$alk * 1e-3 # from meq/L to mol/kg solution  HAS TO BE 10-6 TO FIT -> DID I REGISTER THE WRONG UNIT??? CHECKED AND NO, see "200127 chemical analysis table"
S <- cba.sel$EC_Kje * 1e-3 # from uS/cm to mg/L https://www.landscape.sa.gov.au/mr/publications/measuring-salinity
OrgAlkLiu <- cba.sel$toc * 1e-3 / 12.011 * 0.08 # toc from mg/L to g/L to mol/L, multiplied by fraction of acids 

# compare with pH correction @Liu2020
IS <- cba.sel$cond * 1.3e-5
pHb <- pH - 0.03+0.05*log10(IS)
plot(pH, pHb) # No difference because very low IS

# Is CBA CO2 "dissolved CO2"...
a1  <- aquaenv(pH = pH, CO2 = CO2, SumCO2 = NULL, S = 0, t = temp)
a1b <- aquaenv(pH = pH, CO2 = CO2, SumCO2 = NULL, S = S, t = temp)
plot(a1$TA, a1b$TA) # Salinity plays no role
qplot(y = a1b$TA, x = TA)+geom_abline(intercept = 0, slope = 1)

# Or "total DIC"?
a2 <- aquaenv(pH = pH, SumCO2 = CO2, S = 0, t = temp)
qplot(TA, a2$TA)+geom_abline(intercept = 0, slope = 1)

# Compare: none is good. Use SumCO2 = CO2
qplot()+geom_point(aes(x = TA, y= a1$TA, col = "CO2"))+geom_point(aes(x = TA, y=a2$TA, col = "SumCO2"))
qplot(TA, a1$TA*1e3)+geom_abline(slope = 1, intercept = 0)
qplot(TA, a2$TA*1e4)+geom_abline(slope = 1, intercept = 0)


# determine pH from TA and CO2
a3 <- aquaenv(TA = TA, SumCO2 = CO2, S = S, t = temp)
qplot(pH, a3$pH) # pH superior to 13.5 and 15!

# maybe TA has wrong unit
TAb <- cba.sel$alk *1e-6
a4 <- aquaenv(TA = TAb, SumCO2 = CO2, S = S, t = temp)
qplot(pH, a4$pH)+geom_abline(slope = 1, intercept = 0) # pH too low

# maybe CO2 has wrong unit
CO2b <- cba.sel$CO2 * 1e-3
a5 <- aquaenv(TA = TA, SumCO2 = CO2b, S = S, t = temp)
qplot(pH, a5$pH)+geom_abline(slope = 1, intercept = 0) # pH between 7 and 15

# what about pCO2
a6 <- aquaenv(pH = pH, fCO2 = pCO2, S = 0, t = temp, SumCO2 = NULL)
qplot(TA, a6$TA)+geom_abline(intercept = 0, slope = 1)


```

```{r compute-H2CO3-from-start}
cba <- subset(cba.sel, toc < 40) %>% subset(CO2 <  200)

OrgAlkLiu <- cba$toc * 1e3 / 12.011 * 0.08 # toc from mg/L to umol/L multiplied by fraction of acids -> gives OrgAlk in ueq/L

# values from https://doi.org/10.5268/IW-4.2.694
A1 <- -58.0931
A2 <- 90.5069
A3 <- 22.2940
Temp <- cba$temp # in K
Kh <- exp(A1+A2*100/Temp+A3*log(Temp/100))
fCO2.2019.atm <- 410 * 1e-6 #ppm to atm
H2CO3_eq <- fCO2.2019.atm/Kh # if eq with atm

TA <- cba$alk * 1e-3 # in mol/L
TIC <- cba$CO2 * 1e-3 # in mol/L
TOC <- cba$toc * 1e-3 / 12.011 # in mol/L
pH <- cba$pH_Kje

qplot(TA, TIC, col = TOC)+scale_color_viridis_c()+geom_abline(slope = 1, intercept = 0)

# Considering TIC = H2CO3 + HCO3 (CO3 negligible at low pH)
H2CO3_w <- TIC/(1+(10^(-6.3)/10^(-pH))) # in mol/L
HCO3_w <- H2CO3_w*(10^(-6.3)/10^(-pH))

qplot()+geom_point(aes(x=pH, y = H2CO3_w, col = "H2CO3"))+geom_point(aes(x=pH, y = HCO3_w, col = "HCO3"))

pCO2 <- H2CO3_w*Kh
qplot(TOC, pCO2)+geom_hline(yintercept = fCO2.2019.atm, col = "red")

# Considering TA = H2CO3 + HCO3 (CO3 negligible at low pH)
H2CO3_a <- TA/(1+(10^(-6.3)/10^(-pH))) # in mol/L
HCO3_a <- H2CO3_a*(10^(-6.3)/10^(-pH))

qplot()+geom_point(aes(x=pH, y = H2CO3_a, col = "H2CO3"))+geom_point(aes(x=pH, y = HCO3_a, col = "HCO3"))

pCO2_a <- H2CO3_a*Kh
qplot(TOC, pCO2_a)+geom_hline(yintercept = fCO2.2019.atm, col = "red")

# Find OrgAlk from TA and TIC, but really close to TA

OA <- TA-TIC
qplot(TA,OA)+geom_abline(slope = 1, intercept = 0, col = "red")
plot(TOC,OA) # no relationship with TOC

#In the end we just need to be able to predict TIC based on TA and pH?

plot(TIC,pH)
plot(TIC,TA)
summary(m <- lm(TIC~TA+TOC))

```

```{r catchment-cba, eval = F}
cba.sel <- readRDS("cba.fCO2.rds")
summer.ndvi.2015 <- readRDS("NDVI/100lakes.summer.ndvi.2015.rds")
cba.sf <- merge(cba.sel,summer.ndvi.2015,by = "Lake_ID") %>% st_as_sf()
saveRDS(cba.sf, "cba.sf.rds")
```
...
`r knitr::knit_exit()` \## Test and compare LM and GAM

Selected models: lm1, g1b (on log-transformed data), t1, glm1

```{r test-predict-fCO2-toc, eval = T, cache = T, cache.extra=file.mtime("cba.sf.rds")}
cba.sf <- readRDS("cba.sf.rds")
...
GLM log link = Tweedie log link.

From AIC: GAM/GLM with id link slightly better, but the best is LM.
...
total_filter <- total %>% filter(toc >= 0)
total_filter <- total_filter %>% filter(tp > 0)
total_filter <- total_filter %>% filter(uncertain == FALSE)
...
saveRDS(total_filter, file = "waterchem.rds")
...
wtc <- dplyr::select(waterchem,c("ebint","fCO2","area","perimtot","slope","alt", "latitude","longitude","ndvi.value","arable","bogs","forest","bare","runoff","temp_c","temp","prec","ph","alk","cond","ca","mg","na","k","so4","no3","nh4","cl","f","tp","tn","toc","tndep","tsdep"))

wtc$cp <- wtc$toc / wtc$tp
wtc$cn <- wtc$toc / wtc$tn

wtc$temp_k <- wtc$temp + 273.15

wtc$sat <- NA
wtc$sat[which(wtc$fCO2 < fCO2.1995)] <- -1 # chose -1 instead of 0 to make it easier to interpret the estimates.  
...
```{r over-under-plot}
wtc <- readRDS("wtc.rds")
ggplot(wtc)+geom_point(aes(x = longitude, y = latitude, col = as.factor(sat), size = log10(area)))+
  scale_color_manual(values = c("firebrick","gray"), labels = c("Undersaturated","Supersaturated"), name = "")+
