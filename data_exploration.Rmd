...
wtc <- dplyr::select(total_filter,c("ebint","fCO2","area","perimtot","slope","alt", "latitude","longitude","ndvi.value","arable","bogs","forest","bare","runoff","temp_c","temp","prec","ph","alk","cond","ca","mg","na","k","so4","no3","nh4","cl","f","tp","tn","toc"))
...
```{r compare-over-under, fig.dim = c(5,10)}
library(reshape2)
wtc <- readRDS("wtc.rds")
...
ggplot(filter(wtc.long, variable %in% c("alk","cond","toc","tp","tn","area")))+geom_boxplot(aes(y = log10(value), fill = variable))+
...
glmover <- glm(fCO2~toc, data = oversat, family = Gamma(link = "identity"))
glmunder <- glm(fCO2~toc, data = undersat, family = Gamma(link = "identity"))
summary(glmover)
summary(glmunder)
...
cba.sel <- cba.total %>% dplyr::select(c("Lake_ID","CO2","basin_area_km2","lake_area_km2","latitude","longitude","agriculture","peat","forest","era_runoff_1981_2010","era_temp_1981_2010","era_prec_1981_2010","nilu_ndep_2012_2016","pH","Alkalinity","EC","Ca","Mg","K","Na","Cl","F","SO4","NO3","TOTP","TN","TOC","T","geometry"))
...
ggplot(cba.sel)+geom_point(aes(x=CO2,y=SumCO2))+theme_minimal()
ggplot(cba.sel)+geom_point(aes(x=CO2,y=CO2_ae))+theme_minimal()


ggplot(cba.sel)+geom_point(aes(x=alk, y = fCO2))+theme_minimal()
...
wtc$temp <- wtc$temp + 273

glm2 <- glm(fCO2~toc+cond+temp+tp, data = wtc, family = Gamma(link = "identity"), start = c(1,1,1,1,1))
summary(glm2)

glm3 <- glm(fCO2~toc*sat+cond+temp+tp, data = wtc, family = Gamma(link = "identity"), start = c(1,1,1,1,1,1,1))
summary(glm3)

glm4 <- glm(fCO2~toc+cond+temp+tp*sat, data = wtc, family = Gamma(link = "identity"), start = c(1,1,1,1,1,1,1))
summary(glm4)
...
```{r cba-test-glm, cache = T, cache.extra=file.mtime("wtc.rds")}
cba.sf <- readRDS("cba.sf.rds")
cba.sf$sat <- NA
cba.sf$sat[which(cba.sf$fCO2 < fCO2.2019)] <- -1
cba.sf$sat[which(cba.sf$fCO2 > fCO2.2019)] <- 1

cba.sf$temp <- cba.sf$temp + 273
...
ggplot(cba.sf, aes(x=log10(toc)))+
  ...
  geom_point(aes(y=log10(fp3), col = "Pred GLM"), size = 2, alpha = 0.5)+
  geom_point(aes(y=log10(fplm3), col = "Pred LM"), size = 2, alpha = 0.5)+
...
glm5 <- glm(fCO2~toc*sat+tp, data = wtc, family = Gamma(link = "identity"), start = c(1,1,1,1,1))
summary(glm5)

glm6 <- glm(fCO2~toc*sat+cond, data = wtc, family = Gamma(link = "identity"), start = c(1,1,1,1,1))
summary(glm6)

glm7 <- glm(fCO2~toc*sat+temp, data = wtc, family = Gamma(link = "identity"), start = c(1,1,1,1,1))
summary(glm7)
...
glm8 <- glm(fCO2~toc*sat+cond+tp+ph, data = wtc, family = Gamma(link = "identity"), start = c(1,1,1,1,1,1,1))
summary(glm8)

wtc$fp8 <- predict(glm8, type = "response")

p1 <-ggplot(wtc,aes(x=log10(toc)))+geom_point(aes(y=log10(fCO2)), col = "Black")+
  geom_point(aes(y = log10(fp3), col = "GLM with toc*sat+is+tp+Temp"), size = 2, alpha = 0.5)+
  geom_point(aes(y = log10(fp8), col = "GLM with toc*sat+is+tp+ph"), size = 1, alpha = 0.5)+
  geom_hline(yintercept = log10(fCO2.1995), col = "darkblue") +
  scale_color_manual(values = c("firebrick","lightblue"))+
  theme_minimal()

glm9 <- glm(fCO2~toc*tp+cond+tp, data = wtc, family = Gamma(link = "identity"), start = c(1,1,1,1,1))
summary(glm9)

wtc$fp9 <- predict(glm9, type = "response")

ggplot(wtc,aes(x=log10(toc)))+geom_point(aes(y=log10(fCO2)), col = "Black")+
  geom_point(aes(y = log10(fp3), col = "GLM with toc*sat+is+tp+Temp"), size = 2, alpha = 0.5)+
  geom_point(aes(y = log10(fp9), col = "GLM with toc*tp+is+tp"), size = 1, alpha = 0.5)+
  geom_hline(yintercept = log10(fCO2.1995), col = "darkblue") +
  scale_color_manual(values = c("firebrick","lightblue"))+
  theme_minimal()
