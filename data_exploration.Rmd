...
knitr::opts_chunk$set(echo = T, message = T, warning = F, error = F, fig.align = "center", results = T, collapse = T, cache = T, cache.lazy = F)
...
ggplot(wtc,aes(x=log10(toc)))+geom_point(aes(y=log10(fCO2), col = "Obs"))+
  geom_point(aes(y = log10(fpt), col = "Tweedie"), alpha = 0.5)+
  geom_point(aes(y = log10(fpg), col = "GLM"), alpha = 0.5)+
...
ggplot(wtc,aes(x=log10(toc)))+geom_point(aes(y=log10(fCO2), col = "Obs"))+
  geom_point(aes(y = log10(fpt), col = "Tweedie"), alpha = 0.5)+
  geom_point(aes(y = log10(fptb), col = " Smoothed Tweedie"), alpha = 0.5)+
  annotate(geom = "text",label = paste("AIC tweedie ",round(extractAIC(t2)[2]), sep = "="), x = 0, y = 3.5)+
  annotate(geom = "text",label = paste("AIC s tweedie ",round(extractAIC(t2b)[2]), sep = "="), x = 0, y = 3.7)+
  scale_color_manual(values = c("lightblue","black","firebrick"))+theme_minimal()


ggplot(wtc)+geom_point(aes(x=fCO2, y = fpg, col ="gamma"), size = 2, alpha = 0.5)+
  geom_point(aes(x=fCO2, y = fpt, col ="tweedie"), size = 1, alpha = 0.5)+
 theme_minimal()
```
...
glmsat <- glm(sat~ph+alk+cn, data = wtc, family = binomial)
summary(glmsat)
