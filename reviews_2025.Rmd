...
cba$TA_molL <- cba$Alkalinity*1e-3 # convert from meq/L to mol/L
cba$temp_K <- cba$T + 273.15
cba$pCO2_atm  <- 408.75 * 1e-6 # oct 2019
cba$pH <- cba$pH_Kje # keep the pH measured in Chemistry (not bio)
cba$Hplus <- 10^(-cba$pH)
...
```{r wind-speed-10-years-mean-cba}




u10.2019 <- raster("ERA5/u10_2014_2023.nc") 
u10.2019.mean <- u10.2019 %>% stack %>% mean

u10.2019.sd <- lapply(u10.2019, FUN = function(r1){sum((r1-u10.2019.mean)^2)/length(u10.2019)}) # mauvais resultat
  
  
v10.2019 <- raster("ERA5/v10_2014_2023.nc")

nws.files <- list.files(path = "ERA", pattern = "Wind_WFDE5_CRU_2019", full.names = T) 
nws.raster.crop <- lapply(nws.files, raster) %>% lapply(raster::crop, c(0,35,55,73)) # test with head
...
cba.wind$Sc <- with(cba.wind, 1923.6-125.06*T+4.3733*T^2-0.086781*T^3)
...
ggplot(cba.wind)+geom_point(aes(y = ECO2vp_mcmean, x = Lat))
