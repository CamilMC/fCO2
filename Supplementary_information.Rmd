...
no <- norway %>% 
  dplyr::select(c("survey", "fCO2" , "ECO2", "ECO2_low")) 
  
summary_eco2 <- no %>% group_by(survey) %>% summarize_all(median, na.rm = T) 
kable(summary_eco2, caption = "Median of all variables") %>% kable_styling()
```
...
kable(summary_eco2_ta, caption = "Median of all variables, pH > 5.4") %>% kable_styling()

nlss %>% filter(pH > 5.4) %>% pull(ECO2_ta) %>% median(na.rm = T)
median(nlss$ECO2_toc, na.rm = T)
nlss %>% filter(pH > 5.4) %>% pull(ECO2_ta_low) %>% median(na.rm = T)
median(nlss$ECO2_toc_low, na.rm = T)
```
...
kable(bin_FCO2, digits = 2, col.names = c("A = lake area in km2","median pCO2, uatm","median kCO2, m2/d","median ECO2, g/m2/y", "binned FCO2, MtCO2/y","actual FCO2, MtCO2/y","kCO2, m2/d","median ECO2, g/m2/y", "binned FCO2, MtCO2/y", "actual FCO2, MtCO2/y","median ECO2, g/m2/day", "FCO2, MtCO2/y")) %>% kable_styling() %>% 
  add_header_above(c("Lake class" = 1, " " = 1, "Low estimate" = 4, "High estimate" = 4, "Weyhenmeyer" = 2)) %>%
  group_rows(group_label= "All lakes", start_row = 6, end_row = 6) %>%
  column_spec(column = c(5,6,9,10,12), bold = T)

  

# add a column for FCO2 from median ECO2
```
...
nor$log.lake.area <- nor$lake.area %>% log10()
cut(nor$log.lake.area, breaks = 15) %>% unique()
nor$class.lake <- cut(nor$log.lake.area, breaks = 15, labels = F)


bin_nor <- nor %>% group_by(class.lake) %>% 
  summarise(median_area = median(lake.area), total_area = sum(lake.area), median_pCO2 = median(pCO2_toc)*1e6,
            median_ECO2 = median(ECO2_toc, na.rm = T),
            median_k600 = median(k600, na.rm = T)*24*1e-2,
            actual_FCO2_Mt= sum(ECO2_toc*lake.area*1e6/ 12.011 * 43.99 * 365 * 1e-12, na.rm = T)) %>%
  mutate(prop_area = total_area / sum(total_area) * 100,
         bin_FCO2_Mt = total_area *1e6 * median_ECO2 / 12.011 * 43.99 * 365 * 1e-12,
         bin_FCO2_NOR = inland.waters * prop_area / 100 * median_ECO2 / 12.011 * 43.99 * 365 * 1e-12)

bin_nor <- dplyr::select(bin_nor, c(class.lake, median_area, total_area, prop_area, median_pCO2,median_k600, median_ECO2, actual_FCO2_Mt, bin_FCO2_Mt, bin_FCO2_NOR))

sum_table <- c(0, median(nor$lake.area), sum(nor$lake.area), sum(bin_nor$prop_area),
               median(nor$pCO2_toc)*1e6, median(nor$k600, na.rm = T)*24*1e-2, median(nor$ECO2_toc, na.rm = T), 
               sum(bin_nor$actual_FCO2_Mt),
               sum(bin_nor$bin_FCO2_Mt),
               sum(bin_nor$bin_FCO2_NOR))


kable(rbind(bin_nor, sum_table), digits = 2, 
      caption = paste("Norway. FCO2 in sampled lakes as median(ECO2) * total_area = ", 
                      round(with(nor, median(ECO2_toc, na.rm = T)*sum(lake.area)*1e6/ 12.011 * 43.99 * 365 * 1e-12),2), "MtCO2_eq/y; FCO2 in total Norway as median(ECO2) * inland water =",
                      round(with(nor, median(ECO2_toc, na.rm = T)*inland.waters/ 12.011 * 43.99 * 365 * 1e-12),2), "MtCO2_eq/y", sep = " ")) %>% 
  kable_styling()
...
kable(rbind(bin_fin, sum_table), digits = 2, 
