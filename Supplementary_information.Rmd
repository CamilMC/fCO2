...
kCO2 <- filter(nlss, nation == "Sweden")$kCO2 %>% median(na.rm = T)
...
ECO2_a <- kCO2*(pCO2_a-fCO2.atm.1995)*K0*1e6*12.011 #ECO2 in mg/m2/day
ECO2_b <- kCO2*(pCO2_b-fCO2.atm.1995)*K0*1e6*12.011 #ECO2 in mg/m2/day
ECO2_c <- kCO2*(pCO2_c-fCO2.atm.1995)*K0*1e6*12.011 #ECO2 in mg/m2/day
ECO2_d <- kCO2*(pCO2_d-fCO2.atm.1995)*K0*1e6*12.011 #ECO2 in mg/m2/day

kable(data.frame(lake_type = c("a","b","c","d"), median_DIC_mg.L = c(2.5,2,4.9,4.4), pCO2_atm = c(pCO2_a,pCO2_b,pCO2_c,pCO2_d), ECO2_mgC.m2.day = c(ECO2_a, ECO2_b, ECO2_c, ECO2_d))) %>% kable_styling(bootstrap_options = "bordered" )
```
...
df <- nlss %>% filter(nation == "Sweden", pH > 5.4, TA > 0) 

summary_swepH_1995_toc <- c(min(df$ECO2_toc, na.rm = T), median(df$ECO2_toc, na.rm = T),  max(df$ECO2_toc, na.rm = T), mean(df$ECO2_toc, na.rm = T), sd(df$ECO2_toc, na.rm = T))
...
knitr::kable(summary.df, digits = 2) %>% kable_styling(full_width= T, font_size = 14) %>% 
