...
*`r fig_nums("map-norway", "Lakes sampled during the CBA survey (in red) and during the N112 survey (in red). ")`*
...
```{r autocorrelation-of-residuals}
library(spdep)
...
### What is the impact of the 3 samples with TOC > 30 mg/L? 

```{r fit-glm-toc-inf-30, eval = T}
...
### Traing on CBA dataset and test on N112 dataset
...
### Training on N112 dataset and test on CBA
...
*`r fig_nums("dic.toc.tahplus", "Predicted vs fitted values, Standardized residuals vs fitted values,and measured (red) calculated (blue) pCO2 vs pH for DIC ~ TOC + TA x Hplus")`*
...
*`r fig_nums("dic-toc-ta-invhplus", "Predicted vs fitted values, Standardized residuals vs fitted values,and measured (red) calculated (blue) pCO2 vs pH for DIC~TA")`
...
*`r fig_nums("pCO2-TOC-sd", "pCO2 calculated from TOC, with sd")`*

```{r nls-choose-toc, fid.dim = c(10,20)}
ggarrange(
ggplot()+
...
*`r fig_nums("pco2-NLS-pH-ta", "pCO2 values calculated from TOC for the Northern European Lakes Survey, depending on pH (A) and TA (B)")`*
...
*`r fig_nums("map-pco2-NLS", "Map of pCO2 values calculated from TOC for the Northern European Lakes Survey")`*
...
*`r fig_nums("boxplot-compare", "Comparison of the distribution of TA, TOC, pH and pCO2 (measured or predicted) in the CBA, N112 and NLS surveys")`*
...
### ECO2 for Northern European Lakes Survey
 ...
 *`r fig_nums("summary-eco2-ta","Median of pCO2 and ECO2, pCO2 calculated from TA (excluding samples with pH < 5.4)")`*

### Comparison k600 depending on the method

```{r compare-k600-boxplots}
...
*`r fig_nums("boxplots-k600","Boxplots of the k600 calculated for each survey, low estimate (Cole & Caraco) and high estimate (Vachon & Prairie))")`*
...
*`r fig_nums("k600-lake-area","Calculated ECO2 depending on lake area (log-scale) with low estimate of k600 (in blue, without lake area) and red (including lake area)")`*
 ...
 *`r fig_nums("density-eco2","Comparison of the distribution of low and high estimates of ECO2 for the CBA and N112 surveys")`* 
...
kable(tot, digits = 2, col.names = c(" ","pCO2, uatm", "ECO2 from DIC in gC/m2/day","ECO2 from TOC in gC/m2/day","ECO2 from DIC in gC/m2/day","ECO2 from TOC in gC/m2/day")) %>% 
  kable_styling %>% 
  kableExtra::group_rows(group_label= "CBA", start_row = 1, end_row = 2) %>%
  kableExtra::group_rows(group_label= "N112", start_row = 3, end_row = 4) %>% 
  kableExtra::group_rows(group_label= "NLS", start_row = 5, end_row = 6) %>% 
...
binned.nlss <- nlss %>% group_by(nation, inland.water, lake.area.under100, lake.area.over100, lake.category) %>% 
                        summarise(nb.lakes = n(),
                          ...
                          mutate(FCO2 = ifelse(lake.category != "(100,Inf]",
                                               ECO2*(cat.lake.area/lake.area.under100*inland.water)*1e6/12.011 * 43.99 * 1e-12, #MtCO2/y. ECO2 multiplied by the proportion of lakes belonging to each category in Norway, lake area *1e6 to convert in m2,
                                               ECO2*lake.area.over100*1e6/12.011 * 43.99 * 1e-12),
                                 FCO2_low = ifelse(lake.category != "(100,Inf]", 
                                                   ECO2_low*(cat.lake.area/lake.area.under100*inland.water)*1e6/12.011 * 43.99 * 1e-12,
...
# deprecated bin calculations

```{r bin-area-norway, results = F}
