...
```{r glm-dic-toc-log, fig.dim = c(10,8), results = T, eval = F}

dic.toc <- glm(formula = DIC~TOC, data = norway, family = Gamma(link = "log"))
dic.toc.sum <- summary(dic.toc)$coefficients %>% as.data.frame() 
dic.toc.sp <- rownames(dic.toc.sum)[which(dic.toc.sum$`Pr(>|t|)` < 0.05)] %>% paste(collapse = ", ")
dic.toc.sum <- dic.toc.sum %>% format(scientific = T, digits = 2)
...
nlss$pCO2_toc.ta <- predict(pco2.toc.ta, newdata = nlss, type = "response")

g2 <- ggplot(data = nlss, aes(x = pH))+
  geom_point(aes(y = pCO2_toc.ta, col = "pCO2 modelled with TOC+TA"), alpha = 0.5)+
  geom_hline(yintercept = fCO2.atm.1995, lty = 2)+
  labs(x = "pH", col = "" , y = "pCO2, atm")+
...
ggplot(nlss)+geom_point(aes(x=TOC, y = TA))
ggplot(norway)+geom_point(aes(x=TOC, y = TA))
...
ggplot(nlss)+geom_point(aes(x=TA, y = pCO2_toc.ta, col = TOC))
ggplot(nlss)+geom_point(aes(x=TOC, y = pCO2_toc.ta, col = TA))
ggplot(nlss)+geom_point(aes(x=pH, y = pCO2_toc.ta))
...
```{r evasion-calculated-toc-ta}

nlss$FCO2_toc <- with(nlss, lake.area*1e6*kCO2*((pCO2_toc.ta - fCO2.atm)*K0*1e3)) #-> mol/day, lake.area in m2, dfCO2 in mol/day
...
ggplot()+
  ...
  geom_point(data = nlss, aes(x=long, y = lat, col = log10(pCO2_toc.ta*1e6)), shape = 16, size = 1)+
  labs(col = "fCO2 (uatm), \nlog scale") +
...
ggplot()+
  borders(region = c("Norway", "Sweden","Finland"), fill = "seashell2", col = "seashell2")+xlim(4,35)+ylim(55,72)+
  geom_point(data = subset(nlss, ECO2_toc < 100), aes(x=long, y = lat, col = ECO2_toc), shape = 16, size = 1)+
  geom_point(data = subset(nlss, ECO2_toc > 100), aes(x=long, y = lat), col = "firebrick", shape = 16, size = 1.5)+
