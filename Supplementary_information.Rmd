...
dic.ta.sum <- dic.ta.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
pco2.ta.sum <- pco2.ta.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
dic.toc.sum <- dic.toc.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
pco2.toc.sum <- pco2.toc.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
dic.toc.ta.sum <- dic.toc.ta.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
pco2.toc.ta.sum <- pco2.toc.ta.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
dic.tocHplus.ta.sum <- dic.tocHplus.ta.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
pco2.tocHplus.ta.sum <- pco2.tocHplus.ta.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
dic.taHplus.sum <- dic.taHplus.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
pco2.taHplus.sum <- pco2.taHplus.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
dic.toc.taHplus.sum <- dic.toc.taHplus.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
pco2.toc.taHplus.sum <- pco2.toc.taHplus.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
dic.toc.tainvHplus.sum <- dic.toc.tainvHplus.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
pco2.toc.tainvHplus.sum <- pco2.toc.tainvHplus.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
dic.tocinvHplus.ta.sum <- dic.tocinvHplus.ta.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
pco2.tocinvHplus.ta.sum <- pco2.tocinvHplus.ta.sum %>% mutate(`Pr(>|t|)` = format(`Pr(>|t|)`, scientific = T, digits = 2))
...
ggplot(nlss)+geom_point(aes(x=long, y = lat, col = TOC))+
  borders(regions = c("Norway","Sweden","Finland"))+ylim(55,72)+xlim(0,32)+
...
ggplot(data = nlss, aes(x = pH))+
  geom_point(aes(y = ta_pCO2), alpha = 0.5, col = "firebrick")+
  labs(x = "pH", y = "pCO2 from TA, atm")+
  geom_hline(yintercept = fCO2.atm.1995, lty = 2)+
  facet_zoom(y = ta_pCO2 > 0, zoom.size = 1)+
  theme_bw()
```
...
nlss$FCO2_ta <- with(nlss, lake.area*1e6*kCO2*((ta_pCO2 - fCO2.atm)*K0*1e3)) #-> mol/day, lake.area in m2, dfCO2 in mol/day

nlss$ECO2_ta <- with(nlss, kCO2*(ta_pCO2-fCO2.atm)*K0*1e3 * 12.011) # in gC/m2/day

nlss$mgC.m2.day_ta <- with(nlss, kCO2*(ta_pCO2-fCO2.atm)*K0*1e3 * 12.011 * 1e3) # in mgC/m2/day
